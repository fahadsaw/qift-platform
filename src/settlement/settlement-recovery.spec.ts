...
describe('SETTLE-3b — §7.4 receivable recovery', () => {
  ...
  it('WALKTHROUGH (§25.2 close): assemble QS-B → approve → execute → receivable extinguished by offset, statement enumerates, credit note attached', async () => {
    ...
    const factsNow: CreditNoteFacts = {
      ...
      qiftCreditNoteNumber: note.qiftCreditNoteNumber as string | null,
      netComponent: note.netComponent as number | null,
      reasonCode: note.reasonCode as string | null,
      taxRuleVersion: note.taxRuleVersion as string | null,
      buyerSnapshot: (note.buyerSnapshot as unknown) ?? null,
      issuerSnapshot: (note.issuerSnapshot as unknown) ?? null,
